...
async def buy_item_with_crypto_callback_handler(call: CallbackQuery):
    ...
    value_data = get_item_value(item_name)
    if value_data:
        await buy_handler(call.message, item_price, value_data, item_price, user_id, item_name)
    else:
        await bot.edit_message_text(chat_id=call.message.chat.id,
                                    message_id=msg,
                                    text='❌ Товара нет в наличии',
...
async def check_payment(call: CallbackQuery):
    id, value_data, item_price, user_id, item_name = call.data.split(':')
    await check_invoice_paid(id, call.message, value_data, item_price, user_id, item_name)
...
def register_user_handlers(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(buy_item_with_crypto_callback_handler,
                                       lambda c: c.data.startswith('buy_cryptomus_'))
